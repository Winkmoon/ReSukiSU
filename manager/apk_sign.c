...
static bool verify_cert_hash(const unsigned char *cert, u32 cert_len,
                             u8 *matched_index)
{
...
static int read_asn1_length(const unsigned char *data, int maxlen, int *offset,
                            int *length)
{
    if (*offset >= maxlen)
        return -1;
...
static int find_v1_cert_in_zip(struct file *fp, unsigned char *cert_buf,
                               u32 max_len, u32 *cert_len)
{
        ...
        if (fn_len > 9) {
            ...
            if ((is_rsa || is_dsa) && strncmp(filename, "META-INF/", 9) == 0) {
                if (header.compressed_size > max_len ||
                    header.uncompressed_size > max_len)
                    return -1;

                if (header.compression == 0) {
                    // Stored (uncompressed)
                    if (ksu_kernel_read_compat(fp, cert_buf,
                ...
                if (header.compression == DEFLATE_METHOD) {
                    ...
                    if (ksu_kernel_read_compat(fp, comp_buf,
                                              header.compressed_size, &pos) !=
                        header.compressed_size) {
                        kfree(comp_buf);
                        return -1;
...
reverify_l2_actor(struct dir_context *ctx, const char *name, int namelen,
                  loff_t off, u64 ino, unsigned int d_type)
{
    ...
    if (namelen > l2->pkg_len + 1 && !strncmp(name, l2->pkg_name, l2->pkg_len) &&
        name[l2->pkg_len] == '-') {
        char apk_path[384];
        u8 signature_index = 0;
        snprintf(apk_path, sizeof(apk_path), "%s/%.*s/base.apk", l2->parent_dir,
                namelen, name);
...
static REVERIFY_FILLDIR_RETURN_TYPE
reverify_l1_actor(struct dir_context *ctx, const char *name, int namelen,
                  loff_t off, u64 ino, unsigned int d_type)
{
...

    {
        ...
        snprintf(subdir_path, sizeof(subdir_path), "/data/app/%.*s", namelen,
                name);
